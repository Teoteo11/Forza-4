public class Grid {
    ...
    public boolean youWin(){
        ...
        for(int i=2;i>=0;i--){
            ...
            if(sumPrincipalDiagonal1==4|| sumPrincipalDiagonal1==20 || sumSecondaryDiagonal1==4 || sumSecondaryDiagonal1==20)return true;
        }
        for(int i=0;i<3;i++){
            int maxShift=3+i;
            int shift=0;
            for(int j=6;j>maxShift;j--) {
                ...
                System.out.println(sumSecondaryDiagonal2);
                ++shift;
            }
         ...
         /*for(int i=3;i<6;i++){
            int maxShift=i-3;
            for(int j=0;j<=maxShift;j++){
                sumSecondaryDiagonal1 =myBoard[i][j]+myBoard[i-1][j+1]+myBoard[i-2][j+2]+myBoard[i-3][j+3];
            }
            if(sumSecondaryDiagonal1 == 4 || sumSecondaryDiagonal1 == 20) return true;
        }*/
        for(int j=1;j<4;j++){
            int maxShift=4-j;
